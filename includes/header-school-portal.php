<?php
/**
 * White-label school portal header.
 * Replaces header-module.php when $_SESSION['school_portal_mode'] is active.
 * Shows ONLY school navigation with org branding — no OrbitDesk references.
 *
 * Included by header-module.php when portal mode is detected.
 * The calling page's $moduleNav array (set by modules/school/_nav.php, which every
 * school page requires before header-module.php) is used for navigation directly —
 * no duplicate nav array is hardcoded here.
 */

// ── Auth guard ────────────────────────────────────────────────────
requireLogin();

$portalOrgId = (int)($_SESSION['school_portal_org_id'] ?? 0);
$user        = currentUser();

if ((int)$user['org_id'] !== $portalOrgId) {
    // Org mismatch — boot to portal login
    session_unset(); session_destroy();
    header('Location: /modules/school/portal-login.php'); exit;
}

// ── Branding ──────────────────────────────────────────────────────
$accent   = $_SESSION['school_portal_accent'] ?? '#1A8A4E';
$ptitle   = $_SESSION['school_portal_title']  ?? ($user['org_name'] ?? 'School Portal');
$logoUrl  = $_SESSION['school_portal_logo']   ?? '';
$initial  = strtoupper(substr($ptitle, 0, 1));

// Darken accent for hover/active states
function spAdjust(string $hex, int $amt): string {
    $h = ltrim($hex, '#');
...
$accentDark = spAdjust($accent, -30);
$accentPale = spAdjust($accent, 60) . '22'; // transparent tint

$pageTitle  = ($moduleName ?? 'School') . ' — ' . $ptitle;
...
<div id="hpSidebar">
  ...
  <div class="hp-brand">
    ...
    <div>
      ...
      <div class="hp-brand-sub">School Management</div>
    </div>
  </div>

  <!-- Nav — sourced directly from modules/school/_nav.php's $moduleNav -->
  <nav class="hp-nav" aria-label="School navigation">
    <?php foreach (($moduleNav ?? []) as $item):
      if (!empty($item['divider'])): ?>
      <div class="hp-nav-section"><?= htmlspecialchars($item['label']) ?></div>
      <?php continue; endif;
  ...
  <div class="hp-user-footer">
    ...
    <a href="/modules/school/portal-logout.php" class="hp-logout-btn" title="Sign out">
  ...
  <div class="hp-topbar">
    <div class="d-flex align-items-center gap-3">
      ...
      <div class="hp-breadcrumb">
        <strong><?= htmlspecialchars($moduleName ?? 'School') ?></strong>
