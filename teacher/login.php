 ...
 * URL: /teacher/login.php?org=SLUG
 * Login method: teacher email + password
 */
...
// Already logged in as teacher of this org
if (!empty($_SESSION['tch_id']) && (int)$_SESSION['tch_org_id'] === (int)$org['id']) {
    redirect(APP_URL . '/teacher/index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $orgActive) {
    if (($_POST['csrf_token'] ?? '') !== $csrfToken) {
        ...
        if (!$emailVal || !$password) {
            ...
            try {
                $s = $pdo->prepare(
                    "SELECT * FROM sch_teachers
                     WHERE email=? AND org_id=? AND status='active'
                     LIMIT 1"
                );
            ...
            if (!$teacher || empty($teacher['password_hash'])) {
                $loginError = 'Invalid email or password. Contact your administrator if you need access.';
                ...
                try { $pdo->prepare("INSERT INTO login_attempts (email,ip,success) VALUES (?,?,0)")->execute([$emailVal, $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0']); } catch (Throwable $e) {}
            } elseif (empty($teacher['portal_enabled'])) {
                $loginError = 'Portal access is not enabled for your account. Please contact the school administrator.';
            } else {
                session_regenerate_id(true);
                $_SESSION['tch_id']       = (int)$teacher['id'];
                $_SESSION['tch_name']     = trim($teacher['first_name'] . ' ' . $teacher['last_name']);
                $_SESSION['tch_org_id']   = (int)$org['id'];
                $_SESSION['tch_org_slug'] = $org['slug'];
                $_SESSION['tch_org_name'] = $org['name'];
                $_SESSION['tch_last_act'] = time();

                try {
                    $pdo->prepare("UPDATE sch_teachers SET last_login=NOW() WHERE id=?")->execute([$teacher['id']]);
                } catch (Throwable $e) {}

                setFlash('success', 'Welcome back, ' . e($teacher['first_name']) . '!');
                redirect(APP_URL . '/teacher/index.php');
            }
        }
    }
...
<title>Teacher Login &mdash; <?= e($org['name']) ?></title>
...
<div class="login-split">
  ...
  <div class="login-left">
    <div class="login-left-content">
      ...
      <span class="portal-badge"><i class="fas fa-chalkboard-teacher"></i>Teacher Portal</span>
    ...
    <div class="left-features mt-4">
      <?php foreach ([
        ['fa-clipboard-check', 'Mark daily attendance for your classes'],
        ['fa-graduation-cap',  'Enter and manage exam results'],
        ['fa-book-open',       'Assign and track homework'],
        ['fa-users',           'View your students\' profiles'],
      ] as [$ico, $txt]): ?>
      <div class="feat-item">
  ...
  <div class="login-right">
    <div class="login-form-wrap">
      <div class="form-header">
        ...
        <p>Enter your school email address and password to access your teacher dashboard.</p>
      </div>
      ...
      <form method="POST" autocomplete="on" id="tchLoginForm">
        ...
        <div class="form-field">
          ...
          <div class="input-wrap">
            ...
            <input type="email" id="email" name="email"
                   value="<?= e($emailVal) ?>"
                   placeholder="yourname@school.ac.ke"
        ...
        <button type="submit" class="btn-signin" id="signinBtn">
          <i class="fas fa-sign-in-alt me-2"></i>Sign In to Teacher Portal
        </button>
      </form>

      <div class="login-footer mt-4">
        <p class="mb-1">Forgot your password? <strong>Contact your school administrator</strong> to reset access.</p>
...
document.getElementById('tchLoginForm')?.addEventListener('submit', function() {
