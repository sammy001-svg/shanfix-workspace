...
<div class="row g-3">
    ...
    <div class="card">
      ...
      <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover data-table mb-0">
        ...
        <tr>
          ...
          <td class="small"><?=e($sub['department']??'—')?></td>
          <td><?=$sub['pass_mark']?>%</td>
  ...
  <div class="col-lg-5">
    ...
    <div class="card">
      <div class="card-header"><h6 class="mb-0"><i class="fas fa-list me-2" style="color:<?=$moduleColor?>"></i>All Assignments</h6></div>
      <div class="card-body p-0" style="max-height:320px;overflow-y:auto"><table class="table table-hover mb-0 small">
        <thead class="table-light sticky-top"><tr><th>Class</th><th>Subject</th><th>Teacher</th><th>Periods</th><th></th></tr></thead>
        <tbody>
        <?php if(empty($assignments)):?><tr><td colspan="5" class="text-center text-muted py-3">No assignments.</td></tr>
        <?php else:foreach($assignments as $a):?>
        <tr>
          <td><?=e($a['class_name'])?></td>
          <td class="fw-semibold"><?=e($a['subject_name'])?></td>
          <td><?=$a['first_name']?e($a['first_name'].' '.$a['last_name']):'—'?></td>
